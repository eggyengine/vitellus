...
pub const vkInstance = struct {
    ...
    pub fn init(descriptor: gpu.Instance.Descriptor) hal.Instance.FromPotentialBackendsError!hal.Instance {
        std.log.info("initializing vulkan backend with default descriptor", .{});
    ...
    pub fn initWithDescriptor(allocator: std.mem.Allocator, descriptor: InstanceDescriptor) !hal.Instance {
        std.log.info("initializing vulkan backend: required_extensions={} api_version=0x{x}", .{
        ...
        std.log.debug("loaded vkGetInstanceProcAddr", .{});
        ...
        if (descriptor.enable_validation and !validation_layers_enabled) {
            std.log.warn("vulkan validation requested, but required validation layers are unavailable; continuing without validation", .{});
        }
        ...
        if (validation_layers_enabled) {
            if (debug_utils_enabled) {
                ...
                std.log.warn("vulkan validation enabled, but {s} is unavailable; continuing without debug messenger callback", .{
                    vk.extensions.ext_debug_utils.name,
                });
            }
        }
        std.log.debug("enabled vulkan instance extensions: count={}", .{enabled_extension_count});
        ...
        std.log.debug("created vulkan instance: handle=0x{x}", .{@intFromEnum(instance_handle)});
        ...
        if (debug_utils_enabled) {
            self.debug_messenger = self.instance.createDebugUtilsMessengerEXT(&debug_create_info, null) catch |err| blk: {
                std.log.warn("failed to set up vulkan debug messenger: {s}; continuing without callback", .{@errorName(err)});
    ...
    fn destroyGPU(ptr: *anyopaque) void {
        ...
        if (typed.debug_messenger != .null_handle) {
            std.log.debug("destroying vulkan debug messenger: handle=0x{x}", .{@intFromEnum(typed.debug_messenger)});
        ...
        if (typed.headless_surface) |surface| {
            std.log.debug("destroying cached vulkan headless surface", .{});
            surface.destroy();
        ...
        if (typed.instance_handle != .null_handle) {
            std.log.debug("destroying vulkan instance: handle=0x{x}", .{@intFromEnum(typed.instance_handle)});
        ...
        if (typed.loader) |*loader| {
            std.log.debug("closing vulkan loader", .{});
            loader.close();
            typed.loader = null;
        }
    ...
    fn enumerateAdapters(ptr: *anyopaque, options: gpu.Adapter.RequestOptions) anyerror![]const hal.Adapter {
        ...
        std.log.debug("enumerating vulkan adapters", .{});
    ...
    fn createSurface(
        ...
        const typed: *@This() = @ptrCast(@alignCast(ptr));
        std.log.debug("allocating vulkan surface wrapper", .{});
        ...
        std.log.debug("created vulkan surface wrapper: handle=0x{x}", .{@intFromEnum(surface.handle)});
        return .{
            .ptr = surface,
    ...
    fn requestAdapter(
        ...
        std.log.debug("requesting vulkan adapter", .{});
    ...
    fn requestAdapterInternal(ptr: *anyopaque, options: gpu.Adapter.RequestOptions) anyerror!hal.Adapter {
        ...
        std.log.debug("picking vulkan physical device", .{});
    ...
    fn getOrCreateHeadlessSurface(instance: *@This()) anyerror!hal.Surface {
        if (instance.headless_surface == null) {
            std.log.debug("creating cached vulkan headless surface for adapter selection", .{});
...
fn validationLayersAvailable(allocator: std.mem.Allocator, vkb: BaseWrapper, requested_layers: []const [*:0]const u8) bool {
    ...
    const available_layers = vkb.enumerateInstanceLayerPropertiesAlloc(allocator) catch |err| {
        std.log.warn("failed to enumerate vulkan validation layers: {s}", .{@errorName(err)});
        return false;
    };
    ...
    for (requested_layers) |requested_layer| {
        if (!hasInstanceLayer(available_layers, std.mem.span(requested_layer))) {
            std.log.warn("vulkan validation layer unavailable: {s}", .{requested_layer});
            return false;
        }
    }
...
fn loadDefaultGetInstanceProcAddr(instance: *vkInstance) !vk.PfnGetInstanceProcAddr {
    if (instance.loader == null) {
        std.log.debug("opening vulkan loader: {s}", .{defaultVulkanLoaderName()});
...
fn addRequiredInstanceExtensions(
    ...
    if (required_extensions.len == 0) {
        std.log.debug("no vulkan instance extensions requested", .{});
        return;
    }

    for (required_extensions) |required_extension| {
        if (!hasInstanceExtension(extension_properties, std.mem.span(required_extension))) {
            std.log.debug("missing vulkan instance extension: {s}", .{required_extension});
...
fn addSupportedInstanceExtensions(
    ...
    for (candidate_extensions) |candidate_extension| {
        if (!hasInstanceExtension(extension_properties, std.mem.span(candidate_extension))) {
            std.log.debug("skipping unsupported default vulkan instance extension: {s}", .{candidate_extension});
            continue;
        }
        addInstanceExtension(enabled_extensions, enabled_extension_count, candidate_extension, false) catch |err| {
            std.log.debug("skipping default vulkan instance extension {s}: {s}", .{ candidate_extension, @errorName(err) });
        };
    }
}
...
fn addInstanceExtension(
    ...
    std.log.debug("{s} vulkan instance extension: {s}", .{ if (required) "required" else "default", extension });
}
