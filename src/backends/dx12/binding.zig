...
const sampler_vtable: resource_interface.Sampler.VTable = .{ .deinitFn = destroySampler };
...
pub fn destroyLayout(value: binding.BindGroupLayout) void {
...
pub fn createGroup(ptr: *anyopaque, desc: binding.BindGroupDescriptor) anyerror!binding.BindGroup {
    ...
    if (resource_allocation) |allocation| {
        self.resources = allocation.gpu;
        ...
        for (0..layout_entry.count) |array_element| {
            ...
            switch (layout_entry.kind) {
                .buffer => |buffer_layout| {
                    ...
                    switch (buffer_layout.kind) {
                        ...
                        .storage_read => {
                            ...
                            srv.Format = dx.DXGI_FORMAT_R32_TYPELESS;
                            srv.ViewDimension = dx.D3D12_SRV_DIMENSION_BUFFER;
                        ...
                        .storage_read_write => {
                            ...
                            uav.Format = dx.DXGI_FORMAT_R32_TYPELESS;
                            uav.ViewDimension = dx.D3D12_UAV_DIMENSION_BUFFER;
                ...
                .storage_texture => {
                    ...
                    switch (view.view_dimension) {
                        .d1 => {
                            uav.ViewDimension = dx.D3D12_UAV_DIMENSION_TEXTURE1D;
                            uav.unnamed_0.Texture1D.MipSlice = view.base_mip;
                        },
                        .d1_array => {
                            uav.ViewDimension = dx.D3D12_UAV_DIMENSION_TEXTURE1DARRAY;
                            uav.unnamed_0.Texture1DArray = .{ .MipSlice = view.base_mip, .FirstArraySlice = view.base_layer, .ArraySize = view.layer_count };
                        },
                        .d2 => {
                            uav.ViewDimension = dx.D3D12_UAV_DIMENSION_TEXTURE2D;
                            uav.unnamed_0.Texture2D = .{ .MipSlice = view.base_mip, .PlaneSlice = 0 };
                        },
                        .d2_array, .cube, .cube_array => {
                            uav.ViewDimension = dx.D3D12_UAV_DIMENSION_TEXTURE2DARRAY;
                            uav.unnamed_0.Texture2DArray = .{ .MipSlice = view.base_mip, .FirstArraySlice = view.base_layer, .ArraySize = view.layer_count, .PlaneSlice = 0 };
                        },
                        .d3 => {
                            uav.ViewDimension = dx.D3D12_UAV_DIMENSION_TEXTURE3D;
                            uav.unnamed_0.Texture3D = .{ .MipSlice = view.base_mip, .FirstWSlice = view.base_layer, .WSize = view.layer_count };
                        },
                    }
...
pub fn destroyGroup(value: binding.BindGroup) void {
...
fn compareOp(value: resource_interface.CompareOp) dx.D3D12_COMPARISON_FUNC {
    return switch (value) {
