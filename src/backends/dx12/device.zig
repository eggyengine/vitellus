...
pub const Dx12Device = struct {
    ...
    const vtable: Device.VTable = .{
        ...
        .mapBufferFn = resource.mapBuffer,
        .unmapBufferFn = resource.unmapBuffer,
        .destroyShaderFn = shader.destroy,
        .destroyBufferFn = resource.destroyBuffer,
        .destroyTextureFn = resource.destroyTexture,
        .destroyTextureViewFn = resource.destroyTextureView,
        .destroySamplerFn = binding.destroySampler,
        .destroyBindGroupLayoutFn = binding.destroyLayout,
        .destroyBindGroupFn = binding.destroyGroup,
        .destroyPipelineLayoutFn = pipeline.destroyLayout,
        .destroyGraphicsPipelineFn = pipeline.destroyGraphics,
        .destroyComputePipelineFn = pipeline.destroyCompute,
        .destroyCommandPoolFn = command.destroyPool,
        .resetCommandPoolFn = command.resetPool,
        .destroyCommandBufferFn = command.destroyBuffer,
        .destroyQuerySetFn = command.destroyQuerySet,
        .destroyFenceFn = sync.destroyFence,
        .destroySemaphoreFn = sync.destroySemaphore,
        .fenceValueFn = sync.fenceValue,
        .waitFenceFn = sync.waitFence,
    };
